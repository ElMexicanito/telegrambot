...
# Ensure an event loop exists before creating TelegramClient (fixes RuntimeError: no current event loop)
try:
...
# Esto permite usar valores por defecto en tu PC y variables de entorno en hosting
API_ID = int(os.environ.get("API_ID", 28138561))
API_HASH = os.environ.get("API_HASH", "468799669a0f0402e42f0fb030158c44")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8497377060:AAElqZB2_3uK6FUNoW5J9LyYvE6gObrppko")
ADMIN_ID = int(os.environ.get("ADMIN_ID", 7983990890))
...
@bot.on(events.CallbackQuery(pattern=br'(add|ext)_[0-9]+'))
...
@bot.on(events.CallbackQuery(pattern=br'schedule_[0-9]+'))
...
@bot.on(events.CallbackQuery(pattern=br'cancel_[0-9]+'))
...
@bot.on(events.CallbackQuery(pattern=br'now_[0-9]+'))
